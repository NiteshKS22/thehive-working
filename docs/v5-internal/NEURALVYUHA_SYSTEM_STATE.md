...
**NeuralVyuha** is a "Headless SOC Platform" architecture designed to decouple high-value ingestion, correlation, and search capabilities from the legacy TheHive 4 monolith. It follows the **Strangler Fig Pattern**, allowing v5 services to run alongside v4-LTS without disrupting existing operations.
...
- **NeuralVyuha**: Handles high-volume event processing. Data flows ONE WAY (Ingest -> v5). Future phases may sync v5 data back to v4 or replace v4 modules entirely.
...
### Components Introduced
- `nv-correlation` (Python Worker)
- `nv-group-indexer` (Python Worker)
...
## Runtime Components
1.  **nv-ingest**: Public ingress (AuthZ: JWT).
2.  **nv-dedup**: Worker (Redis).
3.  **nv-correlation**: Worker (Postgres).
4.  **nv-indexer**: Worker (OpenSearch).
5.  **nv-group-indexer**: Worker (OpenSearch).
6.  **nv-query**: Public search API (AuthZ: JWT).
...
### Components Introduced
- `nv-case-engine` (FastAPI): Handles Case mutations (Create, Update, Link).
...
- **CQRS-Lite**: `nv-case-engine` handles Writes. `nv-query` handles Reads (proxying to Postgres for E3).
...
- **NeuralVyuha** is now feature-complete for backend operations.
...
**Status:** In Progress
**Objective:** Bi-directional sync between v4-LTS and NeuralVyuha.
**Implemented Components:**
-   `docs/neural-vyuha/BRIDGE_EVENT_MODEL.md`
-   `neural-vyuha-engine/v4-sync-service` (Consumer Adapter)
**Pending:**
...
**Components:**
-   `neural-vyuha-engine/v5-writeback-publisher`
